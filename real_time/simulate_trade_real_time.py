...
class Options:
    ...
    def run(self):
        ...
        # # Вызов метода update и передача списка всех (All) опционов ->
        # self.df, self.zero_strike_all = self.update(self.option_future_lst)
        # # print(self.df.to_string(max_rows=20, max_cols=25))
        self.update()
        self.predict_close = self.predict_price(Path(fr'../close_best_model.keras'))[0][0] + self.zero_strike
        print(round(self.predict_close, -1), future.last)
        self.predict_high = self.predict_price(Path(fr'../high_best_model.keras'))[0][0] + self.zero_strike
        print(round(self.predict_high, -1), future.high)
        self.predict_low = self.predict_price(Path(fr'../low_best_model.keras'))[0][0] + self.zero_strike
        print(round(self.predict_low, -1), future.low)
