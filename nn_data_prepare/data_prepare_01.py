"""
Подготавливает данные для обучения нейросети на основе исторических данных фьючерсов и опционов.
Читает данные из SQLite-базы по тикеру RTS, объединяет информацию по open interest опционов типа call и put.
Агрегирует открытый интерес по страйкам, нормализует его и строит профиль вокруг ближайшего страйка к цене закрытия.
Для каждой даты формируется "окно" из 21 страйка (10 до и 10 после), с признаками цен и разности интересов.
Применяется MinMax-нормализация, добавляются смещённые ценовые признаки относительно центрального страйка.
Результат сохраняется в pickle-файл для дальнейшего обучения модели.
Используется tqdm для отслеживания прогресса обработки строк.
"""
